...
"""
          ...
          , FlowParagraph "The pattern human→agent→agent→human creates a broken-telephone effect. Errors and misassumptions compound rather than being caught early. Better to maintain human→agent→human cycles, staying in the loop to catch issues before they are passed on."
          ...
          , FlowParagraph "Unlike experienced developers who intuitively filter contextual information based on relevance, agents generally treat all provided context with equal weight. This means developers must become explicit about context boundaries when defining each scope of work."
          , FlowParagraph "Effective context management requires specifying which files to include as reference, which APIs are relevant, which patterns to follow, and which architectural constraints apply. Done properly, this increases the probability that agents generate code that integrates cleanly while avoiding unnecessary coupling to unrelated parts of the codebase. Practically - this means creating a spec/FEATURE.md file in your codebase which you own, and use at the starting point for your agent to implement. Any misunderstandings or missing assumptions can be added to the file over time as the implementation progresses. Typically don't allow your agents to update or write these files (you own them!). Of course you can have your agents review and suggest additions during the design phase."
